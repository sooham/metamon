...
SIMPLE_WM_LOG ?= $(SIMPLE_WM_SAVE_DIR)/$(SIMPLE_WM_STAGE).log
SIMPLE_WM_LR ?= 3e-5
SIMPLE_WM_MIN_LR ?= 3e-6
SIMPLE_WM_LR_WARMUP_UPDATES ?= 2000
SIMPLE_WM_LR_SCHEDULE_UPDATES ?= 200000
SIMPLE_WM_BATCH_SIZE ?= 128
SIMPLE_WM_V_SAMPLING_ALPHA ?= 0.5
SIMPLE_WM_EARLY_STOP_PATIENCE ?= 10
SIMPLE_WM_BETA_KL ?= 0.01
SIMPLE_WM_FREE_BITS ?= 0.02
SIMPLE_WM_ENCODER_TOKEN_MASK_PROB ?= 0
SIMPLE_WM_MEAN_RECON_WEIGHT ?= 0.25
SIMPLE_WM_GRAD_ACCUM_STEPS ?= 1
SIMPLE_WM_GRAD_CLIP ?= 1.0
SIMPLE_WM_MAX_UPDATES ?= 0
SIMPLE_WM_RESUME ?= false
SIMPLE_WM_RESUME_CHECKPOINT ?= $(SIMPLE_WM_SAVE_DIR)/$(SIMPLE_WM_STAGE)_latest.pt
SIMPLE_WM_RESUME_ACTIVE = $(if $(filter true 1 yes,$(SIMPLE_WM_RESUME)),$(if $(wildcard $(SIMPLE_WM_RESUME_CHECKPOINT)),true))
SIMPLE_WM_RESUME_ARGS = $(if $(SIMPLE_WM_RESUME_ACTIVE),--resume_checkpoint $(SIMPLE_WM_RESUME_CHECKPOINT))
SIMPLE_WM_TEE_ARGS = $(if $(SIMPLE_WM_RESUME_ACTIVE),-a)
SIMPLE_WM_ADDITIONAL_UPDATES ?= 0
SIMPLE_WM_UPDATE_BUDGET_ARGS = $(if $(filter-out 0,$(SIMPLE_WM_MAX_UPDATES)),--max_updates $(SIMPLE_WM_MAX_UPDATES),$(if $(filter-out 0,$(SIMPLE_WM_ADDITIONAL_UPDATES)),--additional_updates $(SIMPLE_WM_ADDITIONAL_UPDATES),--max_updates 0))
...
SIMPLE_WM_VAL_MC_SAMPLES ?= 4
SIMPLE_WM_TRAIN_EVAL_SAMPLES ?= 2000
SIMPLE_WM_TRAIN_METRIC_WINDOW ?= 100
SIMPLE_WM_CONSOLE_INTERVAL ?= 100
SIMPLE_WM_WANDB ?= true
SIMPLE_WM_WANDB_PROJECT ?= metamon-simple-world-model
SIMPLE_WM_WANDB_NAME ?=
SIMPLE_WM_WANDB_LOG_INTERVAL ?= $(SIMPLE_WM_CONSOLE_INTERVAL)
SIMPLE_WM_WANDB_ARGS = $(if $(filter false 0 no,$(SIMPLE_WM_WANDB)),--no-wandb,--wandb) $(if $(SIMPLE_WM_WANDB_PROJECT),--wandb_project $(SIMPLE_WM_WANDB_PROJECT)) $(if $(SIMPLE_WM_WANDB_NAME),--wandb_name $(SIMPLE_WM_WANDB_NAME)) --wandb_log_interval $(SIMPLE_WM_WANDB_LOG_INTERVAL)
...
train-simple-world-model:
	@if [ -z "$$TMUX" ]; then \
		...
		tmux new-session -d -s simple-world-model-train "bash -lc 'set -o pipefail; $(MAKE) _train-simple-world-model-inner FORMAT=\"$(FORMAT)\" FORMATS=\"$(FORMATS)\" SIMPLE_WM_STAGE=\"$(SIMPLE_WM_STAGE)\" SIMPLE_WM_CACHE_ROOT=\"$(SIMPLE_WM_CACHE_ROOT)\" SIMPLE_WM_V_CHECKPOINT=\"$(SIMPLE_WM_V_CHECKPOINT)\" SIMPLE_WM_M_CHECKPOINT=\"$(SIMPLE_WM_M_CHECKPOINT)\" SIMPLE_WM_CHECKPOINT=\"$(SIMPLE_WM_CHECKPOINT)\" SIMPLE_WM_RESUME=\"$(SIMPLE_WM_RESUME)\" SIMPLE_WM_RESUME_CHECKPOINT=\"$(SIMPLE_WM_RESUME_CHECKPOINT)\" SIMPLE_WM_COMPILE=\"$(SIMPLE_WM_COMPILE)\" SIMPLE_WM_MAX_CONTEXT_TRANSITIONS=\"$(SIMPLE_WM_MAX_CONTEXT_TRANSITIONS)\" SIMPLE_WM_BATCH_SIZE=\"$(SIMPLE_WM_BATCH_SIZE)\" SIMPLE_WM_V_SAMPLING_ALPHA=\"$(SIMPLE_WM_V_SAMPLING_ALPHA)\" SIMPLE_WM_GRAD_ACCUM_STEPS=\"$(SIMPLE_WM_GRAD_ACCUM_STEPS)\" SIMPLE_WM_LR=\"$(SIMPLE_WM_LR)\" SIMPLE_WM_MIN_LR=\"$(SIMPLE_WM_MIN_LR)\" SIMPLE_WM_LR_WARMUP_UPDATES=\"$(SIMPLE_WM_LR_WARMUP_UPDATES)\" SIMPLE_WM_LR_SCHEDULE_UPDATES=\"$(SIMPLE_WM_LR_SCHEDULE_UPDATES)\" SIMPLE_WM_EARLY_STOP_PATIENCE=\"$(SIMPLE_WM_EARLY_STOP_PATIENCE)\" SIMPLE_WM_BETA_KL=\"$(SIMPLE_WM_BETA_KL)\" SIMPLE_WM_FREE_BITS=\"$(SIMPLE_WM_FREE_BITS)\" SIMPLE_WM_ENCODER_TOKEN_MASK_PROB=\"$(SIMPLE_WM_ENCODER_TOKEN_MASK_PROB)\" SIMPLE_WM_MEAN_RECON_WEIGHT=\"$(SIMPLE_WM_MEAN_RECON_WEIGHT)\" SIMPLE_WM_NUM_WORKERS=\"$(SIMPLE_WM_NUM_WORKERS)\" SIMPLE_WM_GRAD_CLIP=\"$(SIMPLE_WM_GRAD_CLIP)\" SIMPLE_WM_MAX_UPDATES=\"$(SIMPLE_WM_MAX_UPDATES)\" SIMPLE_WM_ADDITIONAL_UPDATES=\"$(SIMPLE_WM_ADDITIONAL_UPDATES)\" SIMPLE_WM_VAL_INTERVAL=\"$(SIMPLE_WM_VAL_INTERVAL)\" SIMPLE_WM_VAL_SAMPLES=\"$(SIMPLE_WM_VAL_SAMPLES)\" SIMPLE_WM_VAL_MC_SAMPLES=\"$(SIMPLE_WM_VAL_MC_SAMPLES)\" SIMPLE_WM_TRAIN_EVAL_SAMPLES=\"$(SIMPLE_WM_TRAIN_EVAL_SAMPLES)\" SIMPLE_WM_TRAIN_METRIC_WINDOW=\"$(SIMPLE_WM_TRAIN_METRIC_WINDOW)\" SIMPLE_WM_CONSOLE_INTERVAL=\"$(SIMPLE_WM_CONSOLE_INTERVAL)\" SIMPLE_WM_EXTRA_ARGS=\"$(SIMPLE_WM_EXTRA_ARGS)\" 2>&1 | tee $(SIMPLE_WM_TEE_ARGS) \"$(SIMPLE_WM_LOG)\"'"; \
		sleep 1; \
		if ! tmux has-session -t simple-world-model-train 2>/dev/null; then \
			echo "ERROR: Training exited immediately; recent log output:"; \
			tail -n 40 "$(SIMPLE_WM_LOG)" 2>/dev/null || true; \
			exit 1; \
		fi; \
		echo ""; \
		echo "  Attach:  tmux attach -t simple-world-model-train"; \
		echo "  Logs:    tail -f $(SIMPLE_WM_LOG)"; \
		echo "  Detach:  Ctrl+B, D"; \
		echo "  Kill:    tmux kill-session -t simple-world-model-train"; \
	else \
...
_train-simple-world-model-inner:
	...
	uv run python -m metamon.simple_world_model.train \
		...
		--v_battle_sampling_alpha $(SIMPLE_WM_V_SAMPLING_ALPHA) \
		--grad_accum_steps $(SIMPLE_WM_GRAD_ACCUM_STEPS) \
		--lr $(SIMPLE_WM_LR) \
		--min_lr $(SIMPLE_WM_MIN_LR) \
		--lr_warmup_updates $(SIMPLE_WM_LR_WARMUP_UPDATES) \
		--lr_schedule_updates $(SIMPLE_WM_LR_SCHEDULE_UPDATES) \
		--early_stop_patience $(SIMPLE_WM_EARLY_STOP_PATIENCE) \
		--beta_kl $(SIMPLE_WM_BETA_KL) \
		--free_bits $(SIMPLE_WM_FREE_BITS) \
		--encoder_token_mask_prob $(SIMPLE_WM_ENCODER_TOKEN_MASK_PROB) \
		--mean_recon_weight $(SIMPLE_WM_MEAN_RECON_WEIGHT) \
		$(SIMPLE_WM_UPDATE_BUDGET_ARGS) \
		--grad_clip $(SIMPLE_WM_GRAD_CLIP) \
		--num_workers $(SIMPLE_WM_NUM_WORKERS) \
		--print_interval $(SIMPLE_WM_CONSOLE_INTERVAL) \
		$(SIMPLE_WM_WANDB_ARGS) \
		$(if $(SIMPLE_WM_CHECKPOINT),--checkpoint $(SIMPLE_WM_CHECKPOINT)) \
		$(SIMPLE_WM_RESUME_ARGS) \
		$(if $(SIMPLE_WM_CONFIG),--config $(SIMPLE_WM_CONFIG)) \
		--val_interval $(SIMPLE_WM_VAL_INTERVAL) \
		--val_samples $(SIMPLE_WM_VAL_SAMPLES) \
		--val_mc_samples $(SIMPLE_WM_VAL_MC_SAMPLES) \
		--train_eval_samples $(SIMPLE_WM_TRAIN_EVAL_SAMPLES) \
		--train_metric_window $(SIMPLE_WM_TRAIN_METRIC_WINDOW) \
