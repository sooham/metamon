...
train-simple-world-model:
	@if [ -z "$$TMUX" ]; then \
		...
		tmux new-session -d -s simple-world-model-train "bash -lc 'set -o pipefail; $(MAKE) _train-simple-world-model-inner FORMAT=\"$(FORMAT)\" FORMATS=\"$(FORMATS)\" SIMPLE_WM_STAGE=\"$(SIMPLE_WM_STAGE)\" SIMPLE_WM_CACHE_ROOT=\"$(SIMPLE_WM_CACHE_ROOT)\" SIMPLE_WM_V_CHECKPOINT=\"$(SIMPLE_WM_V_CHECKPOINT)\" SIMPLE_WM_M_CHECKPOINT=\"$(SIMPLE_WM_M_CHECKPOINT)\" SIMPLE_WM_CHECKPOINT=\"$(SIMPLE_WM_CHECKPOINT)\" SIMPLE_WM_COMPILE=\"$(SIMPLE_WM_COMPILE)\" SIMPLE_WM_MAX_CONTEXT_TRANSITIONS=\"$(SIMPLE_WM_MAX_CONTEXT_TRANSITIONS)\" SIMPLE_WM_BATCH_SIZE=\"$(SIMPLE_WM_BATCH_SIZE)\" SIMPLE_WM_GRAD_ACCUM_STEPS=\"$(SIMPLE_WM_GRAD_ACCUM_STEPS)\" SIMPLE_WM_LR=\"$(SIMPLE_WM_LR)\" SIMPLE_WM_NUM_WORKERS=\"$(SIMPLE_WM_NUM_WORKERS)\" SIMPLE_WM_GRAD_CLIP=\"$(SIMPLE_WM_GRAD_CLIP)\" SIMPLE_WM_MAX_UPDATES=\"$(SIMPLE_WM_MAX_UPDATES)\" SIMPLE_WM_VAL_INTERVAL=\"$(SIMPLE_WM_VAL_INTERVAL)\" SIMPLE_WM_VAL_SAMPLES=\"$(SIMPLE_WM_VAL_SAMPLES)\" SIMPLE_WM_CONSOLE_INTERVAL=\"$(SIMPLE_WM_CONSOLE_INTERVAL)\" SIMPLE_WM_EXTRA_ARGS=\"$(SIMPLE_WM_EXTRA_ARGS)\" 2>&1 | tee \"$(SIMPLE_WM_LOG)\"'"; \
		sleep 1; \
		if ! tmux has-session -t simple-world-model-train 2>/dev/null; then \
