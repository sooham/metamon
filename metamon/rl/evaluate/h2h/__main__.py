"""
Head-to-head evaluation launcher.

Usage:
    python -m metamon.rl.evaluate.h2h \\
        --config h2h_config.yaml \\
        --gpus 0 1 2 3 \\
...
from metamon.rl.evaluate.h2h.config import parse_h2h_config
...
def main():
    ...
    # --- full parser ---
    parser = ArgumentParser(description="Run head-to-head evaluation matrix.")
    parser.add_argument("--config", required=True, help="Path to h2h YAML config.")
    parser.add_argument(
    ...
    matchups = parse_h2h_config(config_path, template_vars=tpl_values)

    if args.dry_run:
        preview_matchups(matchups, mode="h2h", template_values=tpl_values)
        return
