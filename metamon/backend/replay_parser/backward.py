    ...
    def _align_states_actions(self, replay: ParsedReplay):
        ...
        for idx, (turn_t, turn_t1) in enumerate(
            ...
            if opponent_action is None:
                opp_pov_slot = 0
                opp_active = (
                    turn_t1.active_pokemon_2[opp_pov_slot]
                    if self.from_p1_pov
                    else turn_t1.active_pokemon_1[opp_pov_slot]
                )
                if opp_active is not None:
                    ...
                    if reason is not None:
                        if reason in ("recharge", "ability: Truant"):
                            ...
                            move_objs = [
                                m for m in opp_active.moves.values() if m is not None
                            ]
                            if move_objs:
                                chosen = random.choice(move_objs)
                                opponent_action = Action(
            ...
            self._opponent_actionlist.append(opponent_action)
